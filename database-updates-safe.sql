-- Safe database schema updates for Season and Game management
-- This version checks for existing objects before creating them

-- 1. Create seasons table (only if it doesn't exist)
...
-- 2. Create discount_codes table (only if it doesn't exist)
...
-- 3. Add season fields to games table (only if they don't exist)
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'games' AND column_name = 'game_number') THEN
        ALTER TABLE games ADD COLUMN game_number INTEGER;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'games' AND column_name = 'is_individual_sale_allowed') THEN
        ALTER TABLE games ADD COLUMN is_individual_sale_allowed BOOLEAN DEFAULT false;
    END IF;
END $$;

-- 4. Create season_attendees table (only if it doesn't exist)
...
-- 6. Enable RLS (only if not already enabled)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'seasons' AND relrowsecurity = true) THEN
        ALTER TABLE seasons ENABLE ROW LEVEL SECURITY;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'discount_codes' AND relrowsecurity = true) THEN
        ALTER TABLE discount_codes ENABLE ROW LEVEL SECURITY;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'season_attendees' AND relrowsecurity = true) THEN
...
BEGIN
    -- Seasons policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'seasons' AND policyname = 'Anyone can view seasons') THEN
        CREATE POLICY "Anyone can view seasons" ON seasons FOR SELECT USING (true);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'seasons' AND policyname = 'Group members can create seasons') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'discount_codes' AND policyname = 'Anyone can view active discount codes') THEN
        CREATE POLICY "Anyone can view active discount codes" ON discount_codes FOR SELECT 
          USING (is_active = true AND (valid_until IS NULL OR valid_until > NOW()));
    END IF;

    -- Season attendees policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'season_attendees' AND policyname = 'Anyone can view season attendees') THEN
        CREATE POLICY "Anyone can view season attendees" ON season_attendees FOR SELECT USING (true);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'season_attendees' AND policyname = 'Players can join seasons') THEN
