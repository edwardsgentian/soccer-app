-- Add attendance status to game_attendees table
-- This allows players to mark themselves as attending/not attending for games they've paid for

-- Add attendance_status column to game_attendees table
...
-- Add attendance_status column to season_attendees table for individual game attendance
ALTER TABLE season_attendees 
ADD COLUMN IF NOT EXISTS attendance_status VARCHAR(20) DEFAULT 'attending' 
CHECK (attendance_status IN ('attending', 'not_attending'));

-- Create a new table to track individual game attendance for season members
...
-- Create RLS policies for season_game_attendance
